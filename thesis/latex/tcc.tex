...
% Introduction's opening paragraphs; the rest is still to be written.
%
...
home~\cite{bick2026rapid}, even though attitudes toward it still vary widely by age
...
On the models' side, LLM inference is expensive~\cite{chen2026network}, and one way of
making it cheaper is the model cascade: a smaller model answers what it can, and the query
is only passed to a larger one when its answer is not reliable
enough~\cite{chen2024frugalgpt}. The idea predates today's LLMs, having been used to
speed up BERT-sized classifiers~\cite{li2021cascadebert}. What counts as expensive depends on the
setting: the layers a classifier runs, the price of an API call, or the energy spent
answering each query.

Where the models run is a separate question. With edge computing, inference is no longer
confined to the cloud, but spread across end devices, edge nodes, and cloud servers, each
tier further from the user than the one below, and with more computing
power~\cite{chen2026network}. RecServe puts the two ideas together, placing a cascade on
...
Following RecServe's decision rule, \citeonline{pakpahan2026continuum} bring tiered LLM inference into the access network
itself, equipping Optical Network Units (ONUs) and Optical Line Terminals (OLTs) to run
part of the inference, so that a Time and Wavelength
Division Multiplexed PON (TWDM-PON) provides the edge and fog tiers of such a hierarchy.
