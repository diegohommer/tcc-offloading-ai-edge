...
llm.generate() blocks until the GPU has finished, so the window is exact.
  ...
  * boundary = GPU chip/module. Matches config/layer_energy.yaml's declared base.
    Host CPU, DRAM and datacenter PUE are not counted.
...
Writes results/traces/gpu_energy_<model>_<gpu>.json.
...
def main():
    ...
    out = here.parents[2] / "results" / "energy_tests" / f"gpu_energy_{slug}_{gpu_slug}_{stamp}.json"
