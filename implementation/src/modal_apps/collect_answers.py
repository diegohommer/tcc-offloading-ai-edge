...
collected at the precision its energy was (or will be) measured at:
  ...
  onu   Qwen2.5-1.5B-Instruct, GGUF Q4_K_M: the quantisation Cloud to Edge
        measured on a Jetson Orin Nano Super under Ollama (llama.cpp), which is
        the ONU tier's energy source. Same 4-bit weights, loaded here by vLLM's
        GGUF loader instead of Ollama. (Earlier collections used the AWQ build,
        which matched the AGX Orin energy figure; that board is not a realistic
        ONU, see energy_tests.md section 5.)
...
USAGE
-----
    modal run src/modal_apps/collect_answers.py              # all 1,319 test items, all tiers
    LIMIT=20 TIERS=olt modal run src/modal_apps/collect_answers.py   # smoke test

Writes results/energy_tests/gsm8k_zeroshot_<tiers>_n<N>_<UTC>.raw.jsonl, one
record per (tier, query), in the same schema as the existing lb_full.raw.jsonl so
the simulators read it unchanged. The UTC timestamp means no run overwrites
...
def main():
    ...
    out = (here.parents[2] / "results" / "energy_tests"
